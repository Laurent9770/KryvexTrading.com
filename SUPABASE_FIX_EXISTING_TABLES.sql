-- Fix existing tables and add missing columns for Kryvex Trading
...
BEGIN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'trading_pairs') THEN
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'trading_pairs' AND column_name = 'base_asset') THEN
            ALTER TABLE trading_pairs ADD COLUMN base_asset TEXT;
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'trading_pairs' AND column_name = 'quote_asset') THEN
            ALTER TABLE trading_pairs ADD COLUMN quote_asset TEXT;
        END IF;
