...
DECLARE
    ...
    _profiles_table_exists boolean;
    _user_roles_table_exists boolean;
    _user_wallets_table_exists boolean;
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') INTO _profiles_table_exists;
    SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_roles') INTO _user_roles_table_exists;
    SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_wallets') INTO _user_wallets_table_exists;
    
    RAISE NOTICE '';
    RAISE NOTICE '📋 TABLE STATUS:';
    RAISE NOTICE '===============';
    RAISE NOTICE '   profiles table exists: %', _profiles_table_exists;
    RAISE NOTICE '   user_roles table exists: %', _user_roles_table_exists;
    RAISE NOTICE '   user_wallets table exists: %', _user_wallets_table_exists;
    
    -- Check if profile exists (only if table exists)
    IF _profiles_table_exists THEN
    ...
    ELSE
        ...
        _is_admin_exists := false;
        _is_admin_value := false;
        RAISE NOTICE '   Profile Exists: N/A (table missing)';
        RAISE NOTICE '   is_admin column exists in profiles: N/A (table missing)';
    END IF;

    -- Check if user_roles entry exists (only if table exists)
    IF _user_roles_table_exists THEN
    ...
    ELSE
        _role_exists := false;
        RAISE NOTICE '   Admin Role Assigned (user_roles): N/A (table missing)';
    END IF;

    -- Check if user_wallets entry exists (only if table exists)
    IF _user_wallets_table_exists THEN
    ...
    ELSE
        ...
        RAISE NOTICE '   Wallet Exists: N/A (table missing)';
    END IF;
    ...
    IF NOT _profiles_table_exists OR NOT _user_roles_table_exists OR NOT _user_wallets_table_exists THEN
        RAISE NOTICE '⚠️ Some required tables are missing. Please run STEP0_CREATE_TABLES.sql first.';
    ELSIF _profile_exists AND _role_exists AND _wallet_exists AND _is_admin_exists AND _is_admin_value THEN
