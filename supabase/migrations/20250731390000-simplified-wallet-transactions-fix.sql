-- =============================================
-- SIMPLIFIED WALLET TRANSACTIONS FIX
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'wallet_transactions' AND column_name = 'transaction_type' AND table_schema = 'public') THEN
        ALTER TABLE public.wallet_transactions ADD COLUMN transaction_type TEXT DEFAULT 'deposit';
...
-- Step 6: Drop and recreate policies to ensure they work with new columns
...
-- Step 7: Ensure permissions are granted
...
-- Step 9: Migrate data from other tables if not already done
DO $$
DECLARE
